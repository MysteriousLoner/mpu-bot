...
while true; do
    now_epoch=$(date +%s)
    target_epoch=$(date -d "$(date +%Y-%m-%d) 00:51:00" +%s)
    if [ "$now_epoch" -ge "$target_epoch" ]; then
        target_epoch=$(date -d "$(date -d 'tomorrow' +%Y-%m-%d) 00:51:00" +%s)
    fi
    sleep_secs=$((target_epoch - now_epoch))
    echo "[entrypoint] Next run at 00:51 MYT. Sleeping for ${sleep_secs}s..."
    sleep "$sleep_secs"
